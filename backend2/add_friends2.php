...
try {
   ...
   $channel->queue_declare('pending_friendrequest_response_queue', false, true, false, false);


   // Script waiting for messages
   echo " [*] Waiting for messages from RabbitMQ: mysql_pending_friendrequest_response_queue\n";
   ...
   $callback = function($msg) use ($channel) {
       ...
       // Forward the response to pending_friendrequest_response_queue
       $message = new AMQPMessage($friendRequestResponse, ['delivery_mode' => 2]); // Make message persistent
       $channel->basic_publish($message, '', 'pending_friendrequest_response_queue');
       echo " [x] Sent friend request response to RabbitMQ: pending_friendrequest_response_queue\n";
   };


   // Consume messages from the queue: mysql_pending_friendrequest_response_queue
   $channel->basic_consume('mysql_pending_friendrequest_response_queue', '', false, true, false, false, $callback);
